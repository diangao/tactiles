...
5. **Live URL responds** — `curl -fsS https://buildday-harness.vercel.app` returns 2xx, and `curl -fsS https://buildday-harness.vercel.app/api/extract-smiles -X POST -H 'content-type: application/json' -d '...'` returns 200 with a real `smiles` field for a known chemistry input.
